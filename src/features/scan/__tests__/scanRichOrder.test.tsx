...
it('행 구성 — 경고 칩 = 기피 성분만(safe 제외) flex-wrap 컨테이너, 미매칭 = 무썸네일+안내문', () => {
  ...
  // 미매칭 행: 썸네일 无 + notInDb 안내 + [+] 无
  expect(s).toContain('scan.notInDb');
  const imgs = tree.root.findAll((n) => n.props?.source?.uri === 'https://cdn/x.jpg');
  expect(imgs.length).toBeGreaterThanOrEqual(1); // 매칭 행만
  expect(byId(tree, 'add-1').length).toBe(0);
});

it('담기 전환 + 필 카운트 동기 — [+]→스테퍼·added 틴트, 0으로 내리면 필 소멸', () => {
  const tree = render(<Harness dishes={DISHES} />);
