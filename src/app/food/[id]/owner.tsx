...
export default function OwnerConfirm() {
  const { id, ingredient, reason, name } = useLocalSearchParams<{ id: string; ingredient?: string; reason?: string; name?: string }>();
  const router = useRouter();
  const insets = useSafeAreaInsets();
  const bottom = useBottomInset(); // P-055: 안드 내비바 보정
  const { t } = useTranslation();
  const { data } = useOwnerConfirmation(id ?? '', ingredient, reason, name);
