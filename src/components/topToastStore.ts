 ...
 */
export type ToastMsg = { text: string; error?: boolean; key: number };

let listener: ((m: ToastMsg) => void) | null = null;
let seq = 0;

/** 어디서든 호출 — 호스트 미마운트면 조용히 무시(웹·테스트 안전). */
export function showTopToast(text: string, opts?: { error?: boolean }) {
  listener?.({ text, error: opts?.error, key: ++seq });
}
