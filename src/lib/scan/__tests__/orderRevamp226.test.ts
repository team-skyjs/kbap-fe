...
const D = (id: number, risk: 'safe' | 'caution' | 'danger' | 'unable', priceKrw: number | null = null) => ({ id, risk, priceKrw });

describe('② 정렬 — P-354(KB-516): menu(unable 최하단 유지) + 가격 양방향(null 맨 아래)', () => {
  const mixed = [D(0, 'danger', 9000), D(1, 'unable', null), D(2, 'safe', 12000), D(3, 'caution', null), D(4, 'unable', 7000), D(5, 'safe', 9000)];

  it('menu(기본) = 메뉴판 순 유지 + unable만 최하단(false-safe 현행)', () => {
    expect(sortResultDishes(mixed, 'menu').map((d) => d.id)).toEqual([0, 2, 3, 5, 1, 4]);
  });

  it('priceDesc = 가격 높은순, null은 맨 아래(메뉴판 순 유지)', () => {
    expect(sortResultDishes(mixed, 'priceDesc').map((d) => d.id)).toEqual([2, 0, 5, 4, 1, 3]);
  });

  it('priceAsc = 가격 낮은순, null은 맨 아래', () => {
    expect(sortResultDishes(mixed, 'priceAsc').map((d) => d.id)).toEqual([4, 0, 5, 2, 1, 3]);
  });

  it('안정 정렬 — 같은 가격(0·5 = 9000)은 메뉴판 순서 유지', () => {
    const desc = sortResultDishes(mixed, 'priceDesc').map((d) => d.id);
    expect(desc.indexOf(0)).toBeLessThan(desc.indexOf(5));
  });
});
...
describe('①④⑤⑥⑦ 소스 잠금', () => {
  ...
  it('② 세그 배선 — 메뉴판 순 기본·가격 양방향(P-354: safety 소멸), 시트 순서 잠금', () => {
    ...
    expect(scan).toContain("(['menu', 'priceDesc', 'priceAsc'] as ResultSortMode[]).map"); // 메뉴판→높은순→낮은순
    expect(scan).not.toContain('sortSafety');
  });
...
describe('P-230: sortSafety false-safe 교정 · AvoidChip 삭제', () => {
  ...
  it('P-354: sortSafety 키 소멸(10로케일) — 가격 정렬 라벨엔 안전 보장 어휘 금지 유지(K-31 파생)', () => {
    const BANNED = [/safest/i, /안전/, /安全/, /an toàn/i, /безопасн/i, /ปลอดภัย/, /segur/i, /\baman\b/i];
    for (const lang of LOCALES) {
      const scan = (JSON.parse(read(`src/lib/i18n/${lang}.json`)) as { scan: Record<string, string> }).scan;
      expect(scan.sortSafety).toBeUndefined(); // P-354: safety 정렬 소멸
      // 정렬 라벨(가격 양방향)은 순서 서술만 — 안전 보장 뉘앙스 금지
      for (const key of ['sortPriceDesc', 'sortPriceAsc', 'sortMenu']) {
        for (const re of BANNED) expect(scan[key]).not.toMatch(re);
      }
    }
  });
