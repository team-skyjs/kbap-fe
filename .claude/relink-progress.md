...
| 단계 | 상태 | 커밋 | 메모 |
|---|---|---|---|
| KB-67 인증 | 코드완료(검토중) | (다음 커밋) | beTokens+beAuth+client 401 재시도, 교환→newMember 분기, logout/withdraw 실연결. 실기기 검증은 예진(KB-109 재빌드 선행) |
| KB-75 온보딩 제출 | 진행중 | - | KB-110 스텁 교체만. 스킵=빈 배열, 재제출 거부 처리 |
| KB-68 프로필 | 대기 | - | 화면별 PATCH 필드 엄격 분리 |
