...
| KB-68 프로필 | 코드완료(검토중, 반려1건 수정) | 69d9a65+반려수정 | memberAdapter+useMe/useUpdateMe 실연결(세션 있을 때만, 게스트=mock). PATCH는 제공 필드만. 배너 재유도=서버 플래그 우선. 인증 경계 queryClient.clear(). ⚠️ Unit 응답 가드 완화(client) |
...
### 게이팅 실기기 반려 3건 수정 (2026-07-13 밤, KB-78/84)
1. 최초 진입 라우팅: 인트로 게이트 미구현(KB-76 때 deferred) + 세션만료 핸들러가 무조건 /login 강제 → 게스트 진입 불가/불안정.
   - `introSeen.ts`(kbap.introSeen.v1) + `_layout.tsx` 첫 실행 게이트: introSeen 판별까지 스플래시 유지(race 제거) → 미본이면 replace(/intro) 1회. 인트로의 가입/둘러보기/Skip 모두 markIntroSeen.
   - 세션만료: guestMode ON이면 /login 강제 안 함(게스트 강등, 캐시 clear로 화면 재평가). guestMode OFF일 때만 기존 동작.
2. 게스트 스캔 뚫림: 진입로별 가드만 있고 화면 마운트 가드 없음 → scan.tsx에 라우트 자체 가드(게스트=카메라 미마운트, 시트+닫으면 뒤로). 진입로가 늘어도 안전.
3. 리뷰 빈 상태가 잠금보다 먼저: reviews.tsx 분기 순서 `!isGuest && all.length===0`으로 — 게스트는 개수 무관 항상 잠금(0건이어도 lock-pop, ghost 영역 minHeight). 작성 화면(review.tsx)에도 라우트 가드 추가(딥링크 이중 방어).
- 웹 셀프 체크 4/4: 스토리지 초기화→인트로→둘러보기→게스트 홈→재실행 바로 홈 / /scan 딥링크→시트·카메라 미노출 / 0건 음식 리뷰→잠금 / 작성 딥링크→시트. 실기기 3회 반복 안정성은 예진 확인.
