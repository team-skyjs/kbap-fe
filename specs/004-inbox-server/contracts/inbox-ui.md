...
export function useMarkRead(): UseMutationResult<InboxItem, unknown, number, Ctx>; // 항목 단위 낙관·롤백(세대 가드), 목록 GET 없음
...
- `useMarkRead` 컨텍스트 `{ prevRead: boolean | undefined; gen: number }`. `onSuccess` = 응답 항목으로 그 id만 교체(재조회 없음). `onError` = `gen === currentGen()`일 때만 **그 항목만** `prevRead`로 복원. 읽음 처리는 성공·실패 모두 목록 GET 0(Codex #163 1R·2R 반영, 종한 결정 9/17).
