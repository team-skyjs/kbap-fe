...
## R-16. OS 알림 권한 꺼짐 = 배너 + 설정 UI 흐림·조작 불가 (2026-09-14 종한 지시 2회)

- **1차 지시**: "저장된 동의 내역이 그대로 노출되지 않고 문구만" → 토글·캡션 통째로 숨김 + 판정 전 스켈레톤(커밋 dc03256).
- **2차 지시(같은 날)**: "아래 UI를 아예 안 보이게 하지 마셈" → **되돌림**. Decision: 배너 + 아래 설정 UI는 렌더하되 그룹 래퍼에 `opacity 0.4`(기존 `rowDisabled`) + `pointerEvents none`, 토글 행 `disabled`(a11y). 상태로 바뀌는 건 불투명도만(P-151). 판정 전 스켈레톤 게이트도 철회(초기 `'unavailable'` 복귀) — UI가 어차피 보이므로 선노출 문제 없음.
- **Rationale**: 두 지시의 교집합 = "있는 건 보이되 OS를 켜기 전엔 만질 수 없다". 저장값을 흐리게 보여주는 것은 실제 수신 상태와의 괴리를 배너+흐림으로 설명한다. 서버 값 무변.
- **Alternatives considered**: 완전 인터랙티브(원래 코드) — 1차 지시 취지와 어긋남. 완전 숨김(1차) — 2차 지시로 기각.
