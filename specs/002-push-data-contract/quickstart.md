...
| 스위트 | 잠그는 것 (스펙 참조) |
|--------|----------------------|
| `pushAdapter192` | 매핑 5종 + `UNKNOWN_FUTURE` + `NUDGE` = null (US1 1~8, FR-009) · foodId 숫자/문자열 동일 경로 · 탭 시 `onRoute(href, notificationId)` 전달, 부재 시 `undefined` (US3 1~2) · 콜드 스타트 응답 1회 전달 (US3 3) · `Platform.OS='android'`에서 `activity`(MAX)·`news`(HIGH) 각 1회, 이름 = i18n 키, `default` 0회, `'ios'`에서 0회 (US4 1~2) |
...
2. **앱을 백그라운드로 보내거나 종료한 상태에서**(포그라운드 핸들러는 소리 없음 — 범위 밖) Expo push tool 또는 dev 서버에서 `channelId: "news"`, `priority: "high"`, `sound: "default"`, `data: { type: "MEAL_TIME", notificationId: 1 }` 발송(BE 매핑 전이면 수동으로 news 지정).
